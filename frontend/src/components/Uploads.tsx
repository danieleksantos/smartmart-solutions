...
const UploadCard: React.FC<UploadCardProps> = ({ title, endpoint, description, color }) => {
  ...
  return (
    <div className={`p-6 bg-white rounded-lg shadow-md border-t-4 ${color}`}>
      ...
      <div className="flex items-center gap-4">
        ...
        {status === 'uploading' && <span className="text-blue-500 text-sm animate-pulse">Enviando...</span>}
        {status === 'success' && <div className="flex items-center text-green-600 text-sm"><CheckCircle size={16} className="mr-1"/> {msg}</div>}
        {status === 'error' && <div className="flex items-center text-red-600 text-sm"><AlertCircle size={16} className="mr-1"/> {msg}</div>}
      </div>
    </div>
  );
};
...
const Uploads: React.FC = () => {
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">Importação de Dados</h1>
      <p className="text-gray-600 mb-8">Siga a ordem abaixo para evitar erros de vínculo entre os dados.</p>
